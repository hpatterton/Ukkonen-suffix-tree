...
#  Set GRAPHVIZ=True if you want to generate the text that you can cut an
#  paste into the GraphViz editor to produce publication quality trees.
#  If set to False, the program outputs line printer type graphics.
...
class Node:
	...
	def setoutedge(self, key, tupple_key):
		anode, label_start_index, label_end_index, bnode = tupple_key
	...
	def __draw__(rnode, chars, v, ed='#'):
		if(rnode.id == 0):
			if GRAPHVIZ == True:
				print('digraph G {\n'+
					'edge [arrowhead =vee;arrowsize=0.5;fontsize=28;labeloc=\"c\"]\n'+
					'node [label=\"\";height=0.2;width=0.3]\n'+
					'splines =\"spline\"\n'+
					'size =\"10,10\";\n'+
					'node [shape=circle];')
		l = len(chars)
		...
		for k, (parent, s, t, node) in gc:
			...
			if GRAPHVIZ == True:
				if '$' in chars[s:t+1]:
					print(str(node.id)+'[shape = \"point\";color = black;style = filled]') #print(str(node.id)+' [label=\"'+str(node.id)+'\"]') #'[shape = \"point\";color = black;style = filled]'
					print(str(parent.id)+'->'+str(node.id)+' [label=\"'+str(chars[s:t+1])+'\"]')
				else:
					print(str(parent.id) + '->' + str(node.id) + ' [label=\"' + str(chars[s:t + 1]) + '\"]')
			else:
				linkid = ''
...
def build(chars, regularize=False):
	...
	actkey = ''
	actlen = 0
	actnodenumber = actnode.id
	my_number = actnodenumber
...
if __name__ == "__main__":
	text='mississipi$'#'abcabxabcd', 'dedododeeodoeodooedeeododooodoede$', 'ooooooooo', 'mississippi']
